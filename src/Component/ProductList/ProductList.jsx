    ...
    <section className="dessert-Area">
      ...
      {productData.map((product, index) => (
        <div className='allProducts' key={index}>
          <img className='desert-image' src={product.image.desktop} loading="lazy" alt="Deserts images" />
          <button className='add-to-cart-btn px-2 py-1' onClick={() => addToCart(product)}>
            <img className='add-to-card-img' src={addtocart} alt="add to cart" />
            Add to Cart
          </button>
          <div className='product-details mt-2 w-100'>
            <p className='productCategory mt-4'>{product.category}</p>
            <p className='productName'>{product.name}</p>
            <p className='productPrice mb-2'>{`$${product.price}`}</p>
          </div>
        </div>
      ))}
    </section>
